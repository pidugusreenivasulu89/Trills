...
export default function FeedScreen({ navigation }) {
        ...
        const loadPending = async () => {
            try {
                ...
                if (stored) {
                    ...
                    setSentRequests(new Set(pendingList));
                }
            } catch (e) { }
        };
        loadUserData();
    ...
    const handleQuickConnect = async (recipientEmail, userName) => {
        try {
            ...
            const url = ENDPOINTS.CONNECTIONS;
            console.log('Calling API:', url, 'with data:', {
            ...
            console.log('Connection API Response:', response.data);

            // Add to LOCAL storage so Profile can show it immediately even without DB
            try {
            ...
            } catch (e) { console.log('Local storage sync failed'); }

            setSentRequests(prev => new Set([...prev, recipientEmail]));
            Alert.alert('Success', `Connection request sent to ${userName}!`);
        } catch (error) {
            console.log('Quick Connect Error:', error);

            // Still add to local even on network error
            try {
                const storedPending = await AsyncStorage.getItem('pending_connections');
                const pendingList = storedPending ? JSON.parse(storedPending) : [];
                if (!pendingList.includes(recipientEmail)) {
                ...
                <View style={styles.recSection}>
                    <View style={styles.sectionHeader}>
                        ...
                        <TouchableOpacity onPress={() => {
                            console.log('Navigating to Explore from See All');
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.recList}>
                        {recommendedPros.map(pro => (
                            <TouchableOpacity
                                ...
                                onPress={() => {
                                    console.log('Navigating to UserProfile for Pro:', pro.name);
                            ...
                            >
                                ...
                                <TouchableOpacity
                                    ...
                                    onPress={(e) => {
                                        e.stopPropagation(); // Prevent trigger parent profile navigation
                                        console.log('Follow button clicked for:', pro.email);
                                ...
                                >
                                    ...
                                    <Text style={[styles.followText, sentRequests.has(pro.email) && styles.sentText]}>
                                        {sentRequests.has(pro.email) ? 'Request Sent' : 'Connect'}
                                    </Text>
                                </TouchableOpacity>
                            </TouchableOpacity>
                        ))}
                            ...
                            <View key={post.id} style={styles.promoCardContainer}>
                                ...
                                <View style={styles.promoContent}>
                                    ...
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {
                                            console.log('Navigating to Explore from Promo');
                                            navigation.navigate('Explore');
                                        }}
                                    >
                        ...
                        <View key={post.id} style={styles.card}>
                            <View style={styles.postHeaderRow}>
                                <TouchableOpacity
                                    style={styles.author}
                                    onPress={() => {
                                        console.log('Navigating to UserProfile for:', post.user);
                                        navigation.navigate('UserProfile', {
                                            user: post.user,
                                            avatar: post.avatar,
                                ...
                                >
                                    ...
                                    <Text style={styles.connectBtnText}>
                                        {sentRequests.has(post.email) ? 'Request Sent' : 'Connect'}
                                    </Text>
                                </TouchableOpacity>
                            </View>
                            ...
                            <View style={styles.actions}>
                                ...
                                <TouchableOpacity
                                    onPress={() => {
                                        ...
                                        navigation.navigate('Comments', { postId: post.id, user: post.user, content: post.content });
                                    }}
...
const styles = StyleSheet.create({
    ...
    title: { fontSize: 24, fontWeight: '900', color: '#1e293b', fontFamily: 'System' },
    ...
    postImg: { width: '100%', height: 250, borderRadius: 20 },
    actions: { flexDirection: 'row', marginTop: 20, alignItems: 'center', gap: 25 },
